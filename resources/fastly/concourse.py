...
FetchVcl = Literal["generated", "custom", "both"] | bool
...
class FastlyResource(ConcourseResource[FastlyVersion]):
    ...
    def __init__(
        ...
        Args:
            api_token: Fastly API token with at least ``purge_select`` scope for
                purge-only pipelines, or ``global`` scope if VCL fetching is used.
    ...
    def _active_version(
        ...
        active = [v for v in versions if v.get("active") and v.number is not None]
        if not active:
    ...
    def fetch_new_versions(
        ...
        if int(active.number) > int(previous_version.service_version):
            return [current]
    ...
    def download_version(  # noqa: PLR0913
        ...
        Args:
            ...
            fetch_vcl: Controls which VCL content is downloaded.  One of
                ...
                (default — skip VCL fetching entirely).
            vcl_dir: Subdirectory of *destination_dir* into which VCL files are
                written.  Defaults to ``"vcl"``.
            ...
            if fetch_vcl:
                ...
                if fetch_vcl in ("custom", "both"):
                    ...
                    for vcl_file in custom_files:
                        name = vcl_file.get("name") or ""
                        content = vcl_file.get("content") or ""
                        if name:
                            (vcl_root / f"{name}.vcl").write_text(
                                content, encoding="utf-8"
                            )
                            written.append(name)
                        if vcl_file.get("main"):
                            main_name = name
                    if main_name:
